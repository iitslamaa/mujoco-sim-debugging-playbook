...
def main() -> None:
    ...
    run([sys.executable, "scripts/generate_dashboard_snapshot_priorities.py"], env=env)
    run([sys.executable, "scripts/generate_dashboard_snapshot_status_brief.py"], env=env)
    run([sys.executable, "scripts/generate_dashboard_snapshot_lead.py"], env=env)
    run(
        [
            sys.executable,
    ...
    dashboard_snapshot_recovery_forecast = ROOT / "outputs" / "dashboard_snapshots" / "recovery_forecast.json"
    if not dashboard_snapshot_recovery_forecast.exists():
        raise SystemExit(f"Expected dashboard snapshot recovery forecast at {dashboard_snapshot_recovery_forecast}")
    dashboard_snapshot_milestones = ROOT / "outputs" / "dashboard_snapshots" / "milestones.json"
    if not dashboard_snapshot_milestones.exists():
        raise SystemExit(f"Expected dashboard snapshot milestones at {dashboard_snapshot_milestones}")
    ...
    dashboard_snapshot_priorities = ROOT / "outputs" / "dashboard_snapshots" / "priorities.json"
    if not dashboard_snapshot_priorities.exists():
        raise SystemExit(f"Expected dashboard snapshot priorities at {dashboard_snapshot_priorities}")
    dashboard_snapshot_status_brief = ROOT / "outputs" / "dashboard_snapshots" / "status_brief.json"
    if not dashboard_snapshot_status_brief.exists():
        raise SystemExit(f"Expected dashboard snapshot status brief at {dashboard_snapshot_status_brief}")
    dashboard_snapshot_lead = ROOT / "outputs" / "dashboard_snapshots" / "lead.json"
    if not dashboard_snapshot_lead.exists():
        raise SystemExit(f"Expected dashboard snapshot lead at {dashboard_snapshot_lead}")
