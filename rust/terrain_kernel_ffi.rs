...
fn apply_blade_segment(terrain: &mut TerrainGrid, start: BladeState, end: BladeState, soil: SoilConfig) -> f64 {
    ...
    for i in 0..terrain.nx {
        for j in 0..terrain.ny {
            ...
            if !(cross.abs() <= end.width * 0.5 && along >= -distance && along <= distance * 0.35) {
                continue;
            }
...
#[no_mangle]
pub extern "C" fn run_terrain_kernel(repeats: u64, out: *mut KernelStats) -> i32 {
    if out.is_null() || repeats == 0 {
        return -1;
    }
    let result = catch_unwind(AssertUnwindSafe(|| {
        let start = Instant::now();
        let mut kernel_result = KernelResult {
            start_volume: 0.0,
            end_volume: 0.0,
            moved_volume: 0.0,
        };
        for _ in 0..repeats {
            kernel_result = run_kernel_once();
        }
        let elapsed_ms = start.elapsed().as_secs_f64() * 1000.0;
        KernelStats {
            repeats,
            elapsed_ms,
            mean_ms: elapsed_ms / repeats as f64,
            start_volume: kernel_result.start_volume,
            end_volume: kernel_result.end_volume,
            moved_volume: kernel_result.moved_volume,
        }
    }));
    match result {
        Ok(stats) => {
