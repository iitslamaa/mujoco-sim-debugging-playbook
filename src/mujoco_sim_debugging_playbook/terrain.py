...
def apply_blade_pass(
    ...
    previous: BladeState | None = None
    for state in blade_path:
        if previous is None:
            previous = state
            continue
        moved, compacted = _apply_blade_segment(terrain, previous, state, soil)
        moved_volume += moved
...
def _apply_blade_segment(
    ...
    deposit_center_along = distance * 0.9
    deposit_along = rel_x * forward[0] + rel_y * forward[1] - deposit_center_along
    deposit_cross = rel_x * lateral[0] + rel_y * lateral[1]
    spread_x = max(distance * (1.2 + soil.spillover_rate), terrain.xs[1] - terrain.xs[0])
    spread_y = max(end.width * (0.45 + soil.spillover_rate), terrain.ys[1] - terrain.ys[0])
    ...
    if weight_sum > 0.0:
        terrain.heights += weights * (float(np.sum(transport) * terrain.cell_area) / weight_sum)

    return removed_volume, compacted_volume
